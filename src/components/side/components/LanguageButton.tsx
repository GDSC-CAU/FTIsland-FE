  ...
  return (
    <Box sx={{display: 'flex', justifyContent: 'center', paddingTop: 3, paddingBottom: 20}}>
      <Select defaultValue={language} sx={selectBoxStyle}  onChange={handleLanguageChange} 
        ...
        IconComponent={props => (
          <KeyboardArrowDownIcon {...props} style={{ color: '#FF8383' }} />
        )}
        >
        <MenuItem value={"한국어"} sx={itemBoxStyle}>한국어</MenuItem>
        <MenuItem value={"English"} sx={itemBoxStyle}>English</MenuItem>
        <MenuItem value={"中文"} sx={itemBoxStyle}>中文</MenuItem>
        <MenuItem value={"日本語"} sx={itemBoxStyle}>日本語</MenuItem>
      </Select>
      </Box>
  )
}
