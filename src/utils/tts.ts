 ...
 */
export const windowTTS = (text: string, languageCode = 'ko-KR') => {
  window.speechSynthesis.cancel();

  const speechMsg = new SpeechSynthesisUtterance(text);
  speechMsg.rate = 1; // 속도: 0.1 ~ 10
  speechMsg.pitch = 1; // 음높이: 0 ~ 2
  speechMsg.lang = languageCode;
 ...
 */
export const googleTTS = async (text: string, languageCode = 'ko-KR', voiceCode = 'Neural2-C') => {
  const ttsOption = {
    input: {
      text,
    },
